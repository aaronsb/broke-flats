...
- **Hidden things** only show from the side: coins under wide canopies, carports and bus shelters, and the one way through every barrier row. A hedge has a tunnel under a matching roof, a tree line a missing trunk under unbroken canopy, a bus stop a shelter with no back wall, a picket fence a loose board, chain-link a hole cut low, a brick wall a culvert. From above each reads as a solid strip.
- **Powerups** sit in wooden crates, and on a crossing row a crate is always under something — a shop awning, a market stall, a porch, a bough, the corner of a building carried on one pillar. From straight above there is nothing to see; tilt and the item spins above the crate, and a grab while tilted lasts longer. Most awnings have nothing under them. The maze gauntlet keeps its own drops, roofed or in a corridor. Star, hourglass, magnet, whistle, golden egg, mushroom, acorn, chili and tilt, which rolls the whole board and slides the traffic off it.
- **Freight wall**: a slow endless train with one gate always down. Every box car has the same roof, but some have a door open on one side, some on both. In one door and out the other is the way across.
...
- **Weather changes the footing.** Rain darkens the ground and leaves puddles; snow settles as splats on every roof, tree and row until, a minute and a half in, the board is white. On either, three quick hops skid you on: one cell in rain, two on snow. The skid stops at a block, and a car in the skid cell is still a car. The robot is heavy and skids one cell less.
- **Levels** cycle day, sunset, night, rain and snow with forest, residential, city and parking scenery. Night traffic runs on real headlights.
