...
function quad(out, corners, samples, skirt) {
  const [a, b, c] = corners;
...
export function snowLane(lane, world) {
  const { pos, nrm, snow } = surfaceFor(staticBoxes(lane));
