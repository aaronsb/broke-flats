...
// map: a page from townmap.js; at: { r, p } the current spot, or null when the
// level being played is not one of the map's; path: [{ r, p }] walked on this
// page, oldest first; offer: positions on row r + 1 open to pick (past the
// top row, marked along the top edge: they open the next page).
  ...
  for (const row of map.rows) {
    for (const s of row) {
      ...
      parts.push(s.gauntlet
        ? `<rect x="${x - 3.5}" y="${y - 3.5}" width="7" height="7" transform="rotate(45 ${x} ${y})" fill="#f5c518" stroke="#141208" class="${cls}"><title>${esc(s.gauntlet.toUpperCase())} GAUNTLET</title></rect>`
        : `<circle cx="${x}" cy="${y}" r="3.5" fill="${fill}" class="${cls}"><title>${esc(LEVELS[s.district].district.name)}</title></circle>`);
    }
  }
  if (offer && at) for (const p of offer) parts.push(`<circle cx="${X(p)}" cy="${Math.max(3, Y(at.r + 1))}" r="${at.r + 1 < MAP_ROWS ? 6.5 : 3}" class="offer"/>`);
